...
class User(Document):
	...
	def password_reset_mail(self, link):
		self.send_login_mail(_("Password Reset"),
			"password_reset", {"link": link.replace("http://192.168.0.110","https://teamplaner.app")}, now=True)

	def send_welcome_mail_to_user(self):
		from frappe.utils import get_url
		link = self.reset_password()
		subject = "Willkommen beim TeamPlaner"
		...
		self.send_login_mail(subject, "new_user",
				dict(
					link=link.replace("http://192.168.0.110","https://teamplaner.app"),
					site_url=get_url().replace("http://192.168.0.110","https://teamplaner.app")
				))
	...
	def send_login_mail(self, subject, template, add_args, now=None):
		...
		args = {
			...
			'user': self.name,
			'title': subject,
			'login_url': get_url().replace("http://192.168.0.110","https://teamplaner.app"),
